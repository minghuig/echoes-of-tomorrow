extends Node2D
## Transient view-only effects: sparks, shockwave rings, fragments, muzzle
## flashes, dodge afterimages, fading projectile trails. The node's material is
## additive, so overlapping shapes bloom on the dark arena. Purely cosmetic —
## nothing here reads input or touches the sim.
...
## An enemy took a non-fatal hit: a small bright spark cluster in its color.
func enemy_hit(pos: Vector2, color: Color) -> void:
	_burst(pos, Vector2.RIGHT, 5, 80.0, 260.0, PI, color, COLOR_SPARK_HOT)


## An enemy was destroyed: radial fragments, a burst, and expanding rings sized
## to the enemy so heavies read bigger than drones.
func enemy_destroyed(center: Vector2, radius: float, color: Color) -> void:
	for i in 10:
		var frag := Fragment.new()
		_set_life(frag, _rng.randf_range(0.35, 0.7))
		frag.pos = center + Vector2.from_angle(_rng.randf() * TAU) * _rng.randf() * radius
		var out := (frag.pos - center).normalized()
		if out == Vector2.ZERO:
			out = Vector2.from_angle(_rng.randf() * TAU)
		frag.vel = out * _rng.randf_range(120.0, 360.0)
		frag.half = Vector2(_rng.randf_range(2.5, 6.0), _rng.randf_range(2.5, 6.0))
		frag.rot = _rng.randf() * TAU
		frag.rot_speed = _rng.randf_range(-8.0, 8.0)
		frag.color = Color(color, 0.9)
		_effects.append(frag)

	_burst(center, Vector2.RIGHT, 12, 100.0, 460.0, PI, color, COLOR_SPARK_HOT)
	ring(center, COLOR_WHITE, radius * 0.5, radius + 70.0, 0.34, 5.0)
...
func trail_ghost(points: PackedVector2Array, color: Color = COLOR_SPARK) -> void:
